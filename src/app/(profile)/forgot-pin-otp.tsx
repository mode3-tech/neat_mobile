...
export default function ForgotPinOtpScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { otp_id } = await authService.forgotPin();
  ...
  const handleResend = async () => {
    ...
    try {
      if (otpId) {
        await authService.resendForgotPinOtp();
      } else {
        const { otp_id } = await authService.forgotPin();
        setOtpId(otp_id);
      }
  ...
  const handleVerify = async () => {
    if (!otpId || !canVerify || loading) return;
    setLoading(true);
    setError('');
    try {
      const { verification_id } = await authService.verifyForgotPinOtp({
        otp_id: otpId,
        otp_code: otp,
      });
      // Guard against navigating after the user backed out mid-request, and
      // replace (not push) so the consumed-OTP screen drops off the stack —
      // Back from reset-pin then returns straight to the originating screen.
      if (!mountedRef.current) return;
      router.replace({
        pathname: '/(profile)/reset-pin' as any,
        params: { verificationId: verification_id },
      });
    } catch (err) {
